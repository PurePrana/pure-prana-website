...
describe('Homepage User Flow', () => {
  ...
  it('should have proper affiliate links', async () => {
    ...
    const amazonLinks = await page.$$eval('a[href*="amazon.com"]', (links: HTMLAnchorElement[]) => 
    ...
    const relAttribute = await firstAmazonLink?.evaluate((el: Element) => el.getAttribute('rel'))
  ...
  it('should have proper meta tags', async () => {
    ...
    const viewport = await page.$eval('meta[name="viewport"]', (el: Element) => el.getAttribute('content'))
  ...
  it('should be responsive', async () => {
    ...
    const mobileGridClass = await page.$eval('.grid', (el: Element) => el.className)
