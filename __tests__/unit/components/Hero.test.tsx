...
describe('Hero Component', () => {
  ...
  it('renders the tagline', () => {
    render(<Hero />)
    const tagline = screen.getByText(/Where ancient wisdom meets modern science/i)
    expect(tagline).toBeInTheDocument()
  })

  it('renders the description text', () => {
    render(<Hero />)
    const description = screen.getByText(/Every Pure Prana formula is rooted in 5,000 years of Ayurvedic tradition/i)
    expect(description).toBeInTheDocument()
  })

  it('renders the CTA buttons', () => {
    render(<Hero />)
    const shopButton = screen.getByRole('link', { name: /Explore Our Products/i })
    const learnButton = screen.getByRole('link', { name: /View Clinical Research/i })
