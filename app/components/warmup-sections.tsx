...
function Stars({ rating, size = 'md' }: { rating: number; size?: 'sm' | 'md' }) {
  ...
  return (
    <span className="inline-flex items-center gap-0.5" aria-label={`${rating} out of 5 stars`}>
      {[1, 2, 3, 4, 5].map((i) => (
        <svg key={i} className={`${cls} ${i <= Math.round(rating) ? 'text-amber-400' : 'text-gray-600'}`} fill="currentColor" viewBox="0 0 20 20">
...
function PrimeBadge() {
  return (
    <span className="inline-flex items-center gap-1 text-[#00A8E1] font-bold text-sm">
      <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M2 18.5A1.5 1.5 0 013.5 17h17a1.5 1.5 0 010 3h-17A1.5 1.5 0 012 18.5zM14.08 7.41a.5.5 0 00-.86 0L12 10.26l-1.22-2.85a.5.5 0 00-.86 0L8 12h8l-1.92-4.59z"/></svg>
      prime
    </span>
  );
}
...
// ─── Hero ────────────────────────────────────────────────────────────────────
export function Hero({ name, tagline, headline, subhead, asin, imageSlug, rating, reviewCount, price }: ProductPageProps) {
  return (
    <section className="pt-6 pb-8 px-4 max-w-lg mx-auto">
      {/* Brand */}
      <div className="text-center mb-6">
        <p className="text-amber-500 font-semibold text-xs tracking-[0.2em] uppercase">Pure Prana Ayurveda</p>
      </div>
      ...
      {/* Tagline */}
      <p className="text-center text-amber-400/80 text-sm font-medium mb-2">{tagline}</p>

      {/* Headline */}
      <h1 className="text-2xl sm:text-3xl font-extrabold text-white leading-tight text-center mb-3">{headline}</h1>
      <p className="text-gray-400 text-base text-center mb-6 max-w-md mx-auto leading-relaxed">{subhead}</p>
...
// ─── Benefits ────────────────────────────────────────────────────────────────
export function Benefits({ benefits, sectionTitle }: { benefits: Benefit[]; sectionTitle?: string }) {
  return (
    <section className="py-10 px-4 max-w-lg mx-auto">
      <h2 className="text-xl font-bold text-white text-center mb-6">{sectionTitle || 'Why It Works'}</h2>
      <div className="grid gap-4">
        {benefits.map((b, i) => (
          <div key={i} className="flex gap-3.5 items-start bg-white/[0.03] border border-white/[0.06] rounded-2xl p-4">
            <span className="text-2xl flex-shrink-0 mt-0.5">{b.icon}</span>
            <div>
              <h3 className="font-semibold text-white text-sm mb-0.5">{b.title}</h3>
...
export function KeyIngredients({ ingredients }: { ingredients: Ingredient[] }) {
  ...
  return (
    <section className="py-10 px-4 max-w-lg mx-auto">
      <h2 className="text-xl font-bold text-white text-center mb-6">Key Ingredients</h2>
      <div className="grid gap-3">
        {ingredients.map((ing, i) => (
          <div key={i} className="bg-emerald-950/30 border border-emerald-800/20 rounded-xl p-4">
            <p className="text-amber-400 font-semibold text-sm">{ing.name}</p>
            <p className="text-gray-400 text-xs mt-1 leading-relaxed">{ing.desc}</p>
          </div>
        ))}
      </div>
    </section>
  ...
  return (
    <section className="py-10 px-4 max-w-lg mx-auto">
      <div className="bg-gradient-to-br from-white/[0.04] to-white/[0.01] border border-white/[0.06] rounded-2xl p-6 text-center">
        ...
        <p className="text-gray-400 text-sm mb-5">{reviewCount}+ verified Amazon reviews</p>
      ...
      <div className="grid grid-cols-4 gap-2">
        {badges.map((b, i) => (
          <div key={i} className="text-center p-3 rounded-xl bg-white/[0.02] border border-white/[0.04]">
            ...
            <p className="text-[10px] font-medium text-gray-400 leading-tight">{b.label}</p>
          </div>
        ))}
      </div>
    </section>
      ...
      <div className="space-y-2">
        {faqs.map((f, i) => (
          <div key={i} className="border border-white/[0.06] rounded-xl overflow-hidden">
            ...
            >
              {f.q}
              <span className={`ml-2 text-gray-500 transition-transform text-xs ${open === i ? 'rotate-180' : ''}`}>▾</span>
            </button>
            {open === i && (
...
// ─── Sticky Footer CTA ──────────────────────────────────────────────────────
export function StickyFooterCTA({ asin, price, name }: { asin: string; price: string; name: string }) {
  ...
  return (
    <div className={`fixed bottom-0 left-0 right-0 bg-gray-950/95 backdrop-blur-lg border-t border-white/[0.06] p-3 z-50 transition-transform duration-300 ${visible ? 'translate-y-0' : 'translate-y-full'}`}>
...
export function WarmupPage(props: ProductPageProps) {
  return (
    <div className="min-h-screen bg-gray-950 pb-24">
      ...
      <MidCTA asin={props.asin} price={props.price} />
      <StickyFooterCTA asin={props.asin} price={props.price} name={props.name} />
    </div>
  );
}
